...
pub const backends = struct {
    pub const glfw = @import("glfw_impl.zig");
    pub const opengl3 = @import("opengl3_impl.zig");
};

pub const VERSION = "1.90.9";
pub const VERSION_NUM = 19090;
...
pub const ChildFlags_NavFlattened = 256;         // Share focus scope, allow gamepad/keyboard navigation to cross over parent border to this child or between sibling child windows.
...
pub const InputTextFlags_CharsScientific = 4;          // Allow 0123456789.+-*/eE (Scientific notation input)
pub const InputTextFlags_CharsUppercase = 8;           // Turn a..z into A..Z
pub const InputTextFlags_CharsNoBlank = 16;            // Filter out spaces, tabs
// Inputs
pub const InputTextFlags_AllowTabInput = 32;           // Pressing TAB input a '\t' character into the text field
pub const InputTextFlags_EnterReturnsTrue = 64;        // Return 'true' when Enter is pressed (as opposed to every time the value was modified). Consider looking at the IsItemDeactivatedAfterEdit() function.
pub const InputTextFlags_EscapeClearsAll = 128;        // Escape key clears content if not empty, and deactivate otherwise (contrast to default behavior of Escape to revert)
pub const InputTextFlags_CtrlEnterForNewLine = 256;    // In multi-line mode, validate with Enter, add new line with Ctrl+Enter (default is opposite: validate with Ctrl+Enter, add line with Enter).
// Other options
pub const InputTextFlags_ReadOnly = 512;               // Read-only mode
pub const InputTextFlags_Password = 1024;              // Password mode, display all characters as '*', disable copy
pub const InputTextFlags_AlwaysOverwrite = 2048;       // Overwrite mode
pub const InputTextFlags_AutoSelectAll = 4096;         // Select entire text when first taking mouse focus
pub const InputTextFlags_ParseEmptyRefVal = 8192;      // InputFloat(), InputInt(), InputScalar() etc. only: parse empty string as zero value.
pub const InputTextFlags_DisplayEmptyRefVal = 16384;   // InputFloat(), InputInt(), InputScalar() etc. only: when value is zero, do not display it. Generally used with ImGuiInputTextFlags_ParseEmptyRefVal.
pub const InputTextFlags_NoHorizontalScroll = 32768;   // Disable following the cursor horizontally
...
// Callback features
pub const InputTextFlags_CallbackCompletion = 131072;  // Callback on pressing TAB (for completion handling)
pub const InputTextFlags_CallbackHistory = 262144;     // Callback on pressing Up/Down arrows (for history handling)
pub const InputTextFlags_CallbackAlways = 524288;      // Callback on each iteration. User code may query cursor position, modify text buffer.
pub const InputTextFlags_CallbackCharFilter = 1048576; // Callback on character inputs to replace or discard them. Modify 'EventChar' to replace or discard, or return 1 in callback to discard.
pub const InputTextFlags_CallbackResize = 2097152;     // Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to grow. Notify when the string wants to be resized (for string types which hold a cache of their Size). You will be provided a new BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example of using this)
pub const InputTextFlags_CallbackEdit = 4194304;       // Callback on any edit (note that InputText() already returns true on edit, the callback is useful mainly to manipulate the underlying buffer while focus is active)
...
pub const TreeNodeFlags_FramePadding = 1024;          // Use FramePadding (even for an unframed text node) to vertically align text baseline to regular widget height. Equivalent to calling AlignTextToFramePadding() before the node.
pub const TreeNodeFlags_SpanAvailWidth = 2048;        // Extend hit box to the right-most edge, even if not framed. This is not the default in order to allow adding other items on the same line without using AllowOverlap mode.
pub const TreeNodeFlags_SpanFullWidth = 4096;         // Extend hit box to the left-most and right-most edges (cover the indent area).
pub const TreeNodeFlags_SpanTextWidth = 8192;         // Narrow hit box + narrow hovering highlight, will only cover the label text.
pub const TreeNodeFlags_SpanAllColumns = 16384;       // Frame will span all columns of its container table (text will still fit in current column)
pub const TreeNodeFlags_NavLeftJumpsBackHere = 32768; // (WIP) Nav: left direction may move to this TreeNode() from any of its child (items submitted between TreeNode and TreePop)
...
pub const TabBarFlags_DrawSelectedOverline = 64;        // Draw selected overline markers over selected tab
pub const TabBarFlags_FittingPolicyResizeDown = 128;    // Resize tabs when they don't fit
pub const TabBarFlags_FittingPolicyScroll = 256;        // Add scroll buttons when tabs don't fit
pub const TabBarFlags_FittingPolicyMask_ = 384;
pub const TabBarFlags_FittingPolicyDefault_ = 128;
...
pub const DragDropFlags_PayloadAutoExpire = 32;         // Automatically expire the payload if the source cease to be submitted (otherwise payloads are persisting while being dragged)
pub const DragDropFlags_PayloadNoCrossContext = 64;     // Hint to specify that the payload may not be copied outside current dear imgui context.
pub const DragDropFlags_PayloadNoCrossProcess = 128;    // Hint to specify that the payload may not be copied outside current process.
...
pub const Mod_Ctrl = 4096;               // Ctrl (non-macOS), Cmd (macOS)
pub const Mod_Shift = 8192;              // Shift
pub const Mod_Alt = 16384;               // Option/Menu
pub const Mod_Super = 32768;             // Windows/Super (non-macOS), Ctrl (macOS)
pub const Mod_Mask_ = 61440;             // 4-bits
...
pub const InputFlags_None = 0;
pub const InputFlags_Repeat = 1;                   // Enable repeat. Return true on successive repeats. Default for legacy IsKeyPressed(). NOT Default for legacy IsMouseClicked(). MUST BE == 1.
// Flags for Shortcut(), SetNextItemShortcut()
// - Routing policies: RouteGlobal+OverActive >> RouteActive or RouteFocused (if owner is active item) >> RouteGlobal+OverFocused >> RouteFocused (if in focused window stack) >> RouteGlobal.
// - Default policy is RouteFocused. Can select only 1 policy among all available.
pub const InputFlags_RouteActive = 1024;           // Route to active item only.
pub const InputFlags_RouteFocused = 2048;          // Route to windows in the focus stack (DEFAULT). Deep-most focused window takes inputs. Active item takes inputs over deep-most focused window.
pub const InputFlags_RouteGlobal = 4096;           // Global route (unless a focused window or active item registered the route).
pub const InputFlags_RouteAlways = 8192;           // Do not register route, poll keys directly.
// - Routing options
pub const InputFlags_RouteOverFocused = 16384;     // Option: global route: higher priority than focused route (unless active item in focused route).
pub const InputFlags_RouteOverActive = 32768;      // Option: global route: higher priority than active item. Unlikely you need to use that: will interfere with every active items, e.g. CTRL+A registered by InputText will be overridden by this. May not be fully honored as user/internal code is likely to always assume they can access keys when active.
pub const InputFlags_RouteUnlessBgFocused = 65536; // Option: global route: will not be applied if underlying background/void is focused (== no Dear ImGui windows are focused). Useful for overlay applications.
pub const InputFlags_RouteFromRootWindow = 131072; // Option: route evaluated from the point of view of root window rather than current window.
// Flags for SetNextItemShortcut()
pub const InputFlags_Tooltip = 262144;             // Automatically display a tooltip when hovering item [BETA] Unsure of right api (opt-in/opt-out)
...
pub const ConfigFlags_NoMouse = 16;                    // Instruct dear imgui to disable mouse inputs and interactions.
...
pub const ConfigFlags_NoKeyboard = 64;                 // Instruct dear imgui to disable keyboard inputs and interactions. This is done by ignoring keyboard events and clearing existing states.
// [BETA] Docking
pub const ConfigFlags_DockingEnable = 128;             // Docking enable flags.
// [BETA] Viewports
...
pub const Col_TabHovered = 33;                // Tab background, when hovered
pub const Col_Tab = 34;                       // Tab background, when tab-bar is focused & tab is unselected
pub const Col_TabSelected = 35;               // Tab background, when tab-bar is focused & tab is selected
pub const Col_TabSelectedOverline = 36;       // Tab horizontal overline, when tab-bar is focused & tab is selected
pub const Col_TabDimmed = 37;                 // Tab background, when tab-bar is unfocused & tab is unselected
pub const Col_TabDimmedSelected = 38;         // Tab background, when tab-bar is unfocused & tab is selected
pub const Col_TabDimmedSelectedOverline = 39; //..horizontal overline, when tab-bar is unfocused & tab is selected
pub const Col_DockingPreview = 40;            // Preview overlay color when about to docking something
pub const Col_DockingEmptyBg = 41;            // Background color for empty node (e.g. CentralNode with no window docked into it)
pub const Col_PlotLines = 42;
pub const Col_PlotLinesHovered = 43;
pub const Col_PlotHistogram = 44;
pub const Col_PlotHistogramHovered = 45;
pub const Col_TableHeaderBg = 46;             // Table header background
pub const Col_TableBorderStrong = 47;         // Table outer and header borders (prefer using Alpha=1.0 here)
pub const Col_TableBorderLight = 48;          // Table inner borders (prefer using Alpha=1.0 here)
pub const Col_TableRowBg = 49;                // Table row background (even rows)
pub const Col_TableRowBgAlt = 50;             // Table row background (odd rows)
pub const Col_TextSelectedBg = 51;
pub const Col_DragDropTarget = 52;            // Rectangle highlighting a drop target
pub const Col_NavHighlight = 53;              // Gamepad/keyboard: current highlighted item
pub const Col_NavWindowingHighlight = 54;     // Highlight window when using CTRL+TAB
pub const Col_NavWindowingDimBg = 55;         // Darken/colorize entire screen behind the CTRL+TAB window list, when active
pub const Col_ModalWindowDimBg = 56;          // Darken/colorize entire screen behind a modal window, when one is active
pub const Col_COUNT = 57;
...
pub const StyleVar_TableAngledHeadersTextAlign = 26; // ImVec2  TableAngledHeadersTextAlign
pub const StyleVar_ButtonTextAlign = 27;             // ImVec2    ButtonTextAlign
pub const StyleVar_SelectableTextAlign = 28;         // ImVec2    SelectableTextAlign
pub const StyleVar_SeparatorTextBorderSize = 29;     // float     SeparatorTextBorderSize
pub const StyleVar_SeparatorTextAlign = 30;          // ImVec2    SeparatorTextAlign
pub const StyleVar_SeparatorTextPadding = 31;        // ImVec2    SeparatorTextPadding
pub const StyleVar_DockingSeparatorSize = 32;        // float     DockingSeparatorSize
pub const StyleVar_COUNT = 33;
...
pub const ButtonFlags_MouseButtonMask_ = 7;  // [Internal]
...
pub const SliderFlags_WrapAround = 256;          // Enable wrapping around from max to min and from min to max (only supported by DragXXX() functions for now.
...
//   - In Visual Studio w/ Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols inside comments.
//   - In VS Code, CLion, etc.: CTRL+click can follow symbols inside comments.
pub const Dir = c_int;                                                                     // -> enum ImGuiDir              // Enum: A cardinal direction (Left, Right, Up, Down)
...
pub const SortDirection = U8;                                                              // -> enum ImGuiSortDirection    // Enum: A sorting direction (ascending or descending)
...
pub const InputFlags = c_int;                                                              // -> enum ImGuiInputFlags_      // Flags: for Shortcut(), SetNextItemShortcut()
...
pub const Vector_ImGuiTextRange = extern struct {
...
pub const Vector_ImGuiStoragePair = extern struct {
...
pub const IO = extern struct {
    ...
    config_mac_osxbehaviors: bool,                                                          // = defined(__APPLE__) // Swap Cmd<>Ctrl keys + OS X style text editing cursor movement using Alt instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text Start and End using Cmd+Arrows instead of Home/End, Double click selects by word instead of selecting whole text, Multi-selection in lists uses Cmd/Super instead of Ctrl.
    ...
    pub const clearInputKeys = ImGuiIO_ClearInputKeys;                   // Clear current keyboard/gamepad state + current frame text input buffer. Equivalent to releasing all keys/buttons.
    pub const clearInputMouse = ImGuiIO_ClearInputMouse;                 // Clear current mouse state.
};
...
pub const Viewport = extern struct {
    ...
    platform_handle: ?*anyopaque,     // void* to hold higher-level, platform window handle (e.g. HWND, GLFWWindow*, SDL_Window*), for FindViewportByPlatformHandle().
...
pub const PlatformIO = extern struct {
    ...
    platform_get_window_pos: ?*const fn (?*Viewport) callconv(.C) Vec2,                              // N . . . .  // (Use ImGuiPlatformIO_SetPlatform_GetWindowPos() to set this from C, otherwise you will likely encounter stack corruption)
    ...
    platform_get_window_size: ?*const fn (?*Viewport) callconv(.C) Vec2,                             // N . . . .  // Get platform window client area size (Use ImGuiPlatformIO_SetPlatform_GetWindowSize() to set this from C, otherwise you will likely encounter stack corruption)
    ...
    pub const setPlatform_GetWindowPos = ImGuiPlatformIO_SetPlatform_GetWindowPos;   // Set ImGuiPlatformIO::Platform_GetWindowPos in a C-compatible mannner
    pub const setPlatform_GetWindowSize = ImGuiPlatformIO_SetPlatform_GetWindowSize; // Set ImGuiPlatformIO::Platform_GetWindowSize in a C-compatible mannner
};
...
pub const setItemTooltip = ImGui_SetItemTooltip;                                     // set a text-only tooltip if preceding item was hovered. override any previous call to SetTooltip().
...
pub const tableGetHoveredColumn = ImGui_TableGetHoveredColumn;                       // return hovered column. return -1 when table is not hovered. return columns_count if the unused space at the right of visible columns is hovered. Can also use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered) instead.
...
pub const dockSpaceOverViewport = ImGui_DockSpaceOverViewport;                       // Implied dockspace_id = 0, viewport = NULL, flags = 0, window_class = NULL
...
pub const getBackgroundDrawList = ImGui_GetBackgroundDrawList;                       // Implied viewport = NULL
pub const getBackgroundDrawListEx = ImGui_GetBackgroundDrawListEx;                   // get background draw list for the given viewport or viewport associated to the current window. this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
pub const getForegroundDrawList = ImGui_GetForegroundDrawList;                       // Implied viewport = NULL
pub const getForegroundDrawListEx = ImGui_GetForegroundDrawListEx;                   // get foreground draw list for the given viewport or viewport associated to the current window. this draw list will be the top-most rendered one. Useful to quickly draw shapes/text over dear imgui contents.
...
pub const isMouseDragging = ImGui_IsMouseDragging;                                   // is mouse dragging? (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
...
extern fn ImGui_BeginTable(str_id: ?[*:0]const u8, columns: c_int, flags: TableFlags) bool;
extern fn ImGui_BeginTableEx(str_id: ?[*:0]const u8, columns: c_int, flags: TableFlags, outer_size: Vec2, inner_width: f32) bool;
...
extern fn ImGui_TableGetHoveredColumn() c_int;
...
extern fn ImGui_DockSpace(dockspace_id: ID) ID;
extern fn ImGui_DockSpaceEx(dockspace_id: ID, size: Vec2, flags: DockNodeFlags, window_class: ?*const WindowClass) ID;
extern fn ImGui_DockSpaceOverViewport() ID;
extern fn ImGui_DockSpaceOverViewportEx(dockspace_id: ID, viewport: ?*const Viewport, flags: DockNodeFlags, window_class: ?*const WindowClass) ID;
...
extern fn ImGui_GetBackgroundDrawListEx(viewport: ?*Viewport) ?*DrawList;
extern fn ImGui_GetForegroundDrawList() ?*DrawList;
extern fn ImGui_GetForegroundDrawListEx(viewport: ?*Viewport) ?*DrawList;
...
extern fn ImGui_Shortcut(key_chord: KeyChord, flags: InputFlags) bool;
extern fn ImGui_SetNextItemShortcut(key_chord: KeyChord, flags: InputFlags) void;
...
extern fn ImGuiPlatformIO_SetPlatform_GetWindowPos(getWindowPosFunc: ?*const fn (?*Viewport, ?*Vec2) callconv(.C) void) void;
extern fn ImGuiPlatformIO_SetPlatform_GetWindowSize(getWindowSizeFunc: ?*const fn (?*Viewport, ?*Vec2) callconv(.C) void) void;
...
extern fn ImGuiIO_ClearInputMouse(self: *IO) void;
...
extern fn ImGuiTextFilter_ImGuiTextRange_split(self: *const TextFilter_ImGuiTextRange, separator: c_char, out: ?*Vector_ImGuiTextRange) void;
...
extern fn ImColor_HSV(h: f32, s: f32, v: f32, a: f32) Color;
